...
define(
	...
	[
		...
		'modules/caret.jquery' //jquery plugin
	],
	...
	function(
		...
		AutoCompleter.prototype = {
			...
			init: function(){
				...
				this.elResults = $('<div class="'+this.config.resultsClassName+'">').css({
					position: 'absolute',
					display: 'none'
				});
				
				this.wrapper.append( this.elResults );
			...
			setOptions: function(config){
				...
				this.config = $.extend( this.config || {
					...
					// element that holds the autocomplete suggest box
					wrapper: 'body',
			...
			positionResults: function() {
				...
				var par = this.wrapper.offset()
					,offset = this.el.offset()
					,align = this.config.align
					;
				
				this.elResults
					.css({ left: '', right: ''}) //reset
					.css( 'top', offset.top - par.top + this.el.outerHeight() )
					.css( align,
							align === 'left'?
								offset[align] - par.left :
								this.wrapper.width() - par.left - offset.left - this.el.outerWidth()
						)
					;
			},
			...
			bindEvents: function(){
				...
				this.el.on({
					...
					select: function(e){
						...
						if ( self.to.activate && !self.machineSelect ){
